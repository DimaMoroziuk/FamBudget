...
export default function Income() {
  ...
  return (
    <React.Fragment>
          ...
          {data.map((row) => {
            ...
            return (
              <TableRow key={row.id}>
                ...
                component="button" disabled={row.accessType !== 1} onClick={() => history.push(`/income/add/${row.id}`)} sx={{ mt: 3 }}>
          Edit
        </Link></TableCell>
                <TableCell align="right"><Link color={(row.accessType !== 1 ? "black" : "primary")} underline={(row.accessType !== 1 ? "none" : "always")} 
                component="button" disabled={row.accessType !== 1} onClick={() => deleteIncome(row)} sx={{ mt: 3 }}>
          Delete
        </Link></TableCell>
              </TableRow>
            )
      ...
      <Link color="primary" href="/income/add" sx={{ mt: 20 }}>
        Add new Income
      </Link>
              <Button
                onClick={handleSave}
